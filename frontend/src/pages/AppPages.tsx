...
export function DashboardPage() {
  ...
  const stats = [
    ...
    ["OCR Extracted", docs.filter((d) => d.status === "ANALYSED").length],
  ];

  return (
    <>
      <p className="text-sm font-semibold text-brand-600">Your workspace</p>
      <h1 className="mt-1 text-3xl font-bold">Welcome back</h1>
      <p className="mt-2 text-slate-500">
        Upload a prescription or medical report to extract all readable text using OCR.
      </p>
      ...
      <div className="mt-7 rounded-2xl bg-brand-600 p-7 text-white md:flex md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold">Ready to extract text from a document?</h2>
          <p className="mt-2 max-w-xl text-brand-100">
            Upload an image or document to run OCR and retrieve all readable text.
          </p>
        </div>
  ...
  return (
    <div className="flex items-center gap-3 p-4">
      ...
      <span className="hidden rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600 sm:inline">
        {d.status === "ANALYSED" ? "Extracted" : d.status}
      </span>
      <Link
...
export function UploadPage() {
  ...
  const go = async () => {
    ...
    try {
      ...
      await documentApi.ocr(uploaded.data.data.id);

      // 3. Navigate to the Extracted Text view
      nav(`/documents/${uploaded.data.data.id}`);
    } catch {
      setError("Upload or OCR text extraction failed. Please try again.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="mt-2 text-slate-500">
        Choose a prescription or medical report. Upload the file and run OCR to extract all readable text.
      </p>
      <div className="mt-7 card p-6">
        ...
        <label
          ...
          className="mt-5 flex cursor-pointer flex-col items-center rounded-2xl border-2 border-dashed border-brand-200 bg-brand-50/50 p-10 text-center hover:bg-brand-50 transition"
        >
        ...
        {file && (
          <div className="mt-4 flex items-center justify-between rounded-xl bg-slate-50 p-4 border border-slate-100">
            <div>
        ...
        <p className="mt-3 text-center text-xs text-slate-500">
          This step performs pure OCR character extraction. No medical interpretation, symptom detection, or LLM processing is applied.
        </p>
      </div>
    </div>
  );
...
export function DocumentsPage() {
  ...
  const remove = async (id: number) => {
    if (!confirm("Delete this document and its extracted text?")) return;
      ...
      <div className="card mt-7 divide-y">
        ...
        {!docs.length && (
          <div className="p-10 text-center">
            ...
            <p className="mt-3 text-slate-500">No saved documents yet.</p>
            <Link className="btn-primary mt-4" to="/upload">
              Upload a document
            </Link>
          </div>
        )}
...
export function ResultsPage() {
  ...
  const handleRunOcr = async () => {
    ...
    try {
      ...
      toast("OCR text extraction completed!");
      loadDocument();
    } catch {
      toast("OCR extraction failed. Please try again.");
    } finally {
      setOcrBusy(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 text-slate-500">
        <LoaderCircle className="animate-spin" /> Loading document…
      </div>
    );
  }
  ...
  if (!doc) {
    return (
      <div className="card p-8 text-center max-w-lg">
        <p className="text-slate-600 font-medium">Document not found.</p>
        <Link to="/documents" className="btn-primary mt-4 text-sm">
          Return to documents
        </Link>
      </div>
    );
  ...
  const ocr = doc.analysis?.structuredResult as OcrResult | undefined;
  const rawText = ocr?.rawText || doc.analysis?.summary || "";
  const lines = ocr?.lines || (rawText ? rawText.split("\n").map((l) => l.trim()).filter(Boolean) : []);
  const wordsCount =
    ocr?.wordsCount ?? (rawText ? rawText.split(/\s+/).filter(Boolean).length : 0);
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    toast("Extracted text copied to clipboard!");
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `${doc.originalName.replace(/\.[^/.]+$/, "")}-extracted-text.txt`;
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {doc.originalName} · {doc.documentType === "PRESCRIPTION" ? "Prescription" : "Medical Report"} · Uploaded{" "}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="card p-4">
          <p className="text-xs text-slate-400 font-semibold uppercase">Words Detected</p>
          <p className="mt-1 text-2xl font-extrabold text-slate-900">{wordsCount}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-slate-400 font-semibold uppercase">Total Lines</p>
          <p className="mt-1 text-2xl font-extrabold text-slate-900">{linesCount}</p>
        </div>
        <div className="card p-4">
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-slate-900">Raw Extracted Text</h2>
          </div>
        ...
        {rawText ? (
            ...
            {viewMode === "lines" && (
              <div className="bg-slate-50 rounded-xl border border-slate-200 divide-y divide-slate-100 overflow-x-auto max-h-[600px] overflow-y-auto">
                {lines.map((line, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-4 px-4 py-2 hover:bg-white transition text-xs sm:text-sm font-mono"
                  >
                    <span className="w-8 shrink-0 select-none text-right font-semibold text-slate-400">
                      {idx + 1}
                    </span>
                    <span className="text-slate-800 break-words select-text flex-1">
                      {line}
                    </span>
                  </div>
                ))}
          ...
          <div className="text-center py-12 px-4">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-700">
              No text has been extracted yet for this document.
            </p>
            <p className="mt-1 text-xs text-slate-400 max-w-md mx-auto">
              Run OCR processing to scan the image and extract all readable text.
            </p>
            <button
              disabled={ocrBusy}
            ...
            >
              {ocrBusy ? (
                <>
                  <LoaderCircle className="animate-spin" size={16} /> Running OCR…
                </>
              ) : (
        ...
        <div className="mt-6 rounded-xl bg-slate-100 p-4 text-xs text-slate-600 border border-slate-200/60 flex items-start gap-2.5">
