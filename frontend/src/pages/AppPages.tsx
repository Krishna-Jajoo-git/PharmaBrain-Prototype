    ...
    <>
      ...
      <div className="mt-7 rounded-2xl bg-brand-600 p-7 text-white md:flex md:items-center md:justify-between">
        ...
        <div className="mt-5 flex gap-3 md:mt-0">
          <Link
            className="rounded-xl bg-white px-4 py-2.5 font-semibold text-brand-700"
            to="/upload?type=PRESCRIPTION"
          >
            Upload Prescription
          </Link>
          <Link
            className="rounded-xl border border-brand-300 px-4 py-2.5 font-semibold"
...
function DocumentRow({ d, onDelete }: { d: Document; onDelete?: () => void }) {
  return (
    <div className="flex items-center gap-3 p-4">
      ...
      >
        {d.analysis ? "View Summary" : "Pending"}
      </Link>
      {onDelete && (
        <button
          onClick={onDelete}
...
export function UploadPage() {
  ...
  const go = async () => {
    ...
    try {
      ...
      await documentApi.analyse(uploaded.data.data.id);
    ...
    <div className="max-w-2xl">
      ...
      <div className="mt-7 card p-6">
        ...
        <label
          ...
          className="mt-5 flex cursor-pointer flex-col items-center rounded-2xl border-2 border-dashed border-brand-200 bg-brand-50/50 p-10 text-center"
        >
