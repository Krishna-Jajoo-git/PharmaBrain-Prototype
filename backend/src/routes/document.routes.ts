...
router.post("/upload", upload.single("file"), async (req: AuthRequest, res, next) => {
  try {
    if (!req.file) return res.status(400).json({ success: false, message: "Choose a JPG, PNG, or PDF file up to 10 MB." });
    ...
    if (process.env.VERCEL) {
      if (process.env.BLOB_READ_WRITE_TOKEN && req.file.buffer) {
        const blob = await put(`pharmabrain/${randomUUID()}-${req.file.originalname}`, req.file.buffer, { access: "public", token: process.env.BLOB_READ_WRITE_TOKEN });
        storedFilename = blob.pathname;
        filePath = blob.url;
      } else if (req.file.buffer) {
        // Fallback for Vercel when Blob Token is not configured: store as data URL so analysis & prototype function seamlessly
